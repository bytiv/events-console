...
State lives in the browser: one `localStorage` key (`dotment-control-center`) for the data, one cookie (`dcc_session`) for sign-in. The seed loads on first visit only. To reset, clear site data for the origin.
